...
class SpircHandler {
 public:
  ...
  // Starts playback locally (without an external Connect controller). Builds a
  // queue from the provided track URIs, marks this device active and begins
  // playing at startIndex. Used by the on-Vita browser/search.
  void loadAndPlayTracks(const std::vector<std::string>& trackUris,
                         const std::string& contextUri, int startIndex = 0);
