...
class QueuedTrack {
 public:
  ...
  const std::vector<uint8_t>& getAudioKey() const { return audioKey; }
...
class TrackQueue : public bell::Task {
 public:
  ...
  bool peekHeadTrackInfo(TrackInfo &out);
  bool getCurrentPlayingTrack(std::shared_ptr<QueuedTrack>& out);
