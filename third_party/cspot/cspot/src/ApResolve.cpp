...
std::string ApResolve::fetchFirstSpclientAddress() {
  auto request =
      bell::HTTPClient::get("https://apresolve.spotify.com/?type=spclient");
  std::string_view responseStr = request->body();

#ifdef BELL_ONLY_CJSON
  cJSON* json = cJSON_Parse(responseStr.data());
  auto host = std::string(cJSON_GetArrayItem(cJSON_GetObjectItem(json, "spclient"),
                                             0)
                              ->valuestring);
  cJSON_Delete(json);
  return host;
